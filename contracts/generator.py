...
def _write_dbt_yaml(
    ...
    """Write dbt schema.yml with full contract rule mapping and return the output path."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for clause in clauses:
        col: dict[str, Any] = {
            "name": clause.field_path,
            "description": clause.description or f"Column {clause.field_path}",
            "tests": [],
        }
        ...
        # unique → unique: true (only for _id fields)
        if clause.field_path.endswith("_id") and clause.required:
            col["tests"].append("unique")

        # accepted_values → enum constraint
        if clause.enum:
            col["tests"].append({
                "accepted_values": {
                    "values": clause.enum,
        ...
        # range check → dbt_utils.expression_is_true
        if clause.minimum is not None or clause.maximum is not None:
            parts = []
            if clause.minimum is not None:
                parts.append(f"{clause.field_path} >= {clause.minimum}")
            if clause.maximum is not None:
                parts.append(f"{clause.field_path} <= {clause.maximum}")
            ...
            col["tests"].append({
                "dbt_utils.expression_is_true": {
                    ...
                    "meta": {
                        "contract_clause": (
                            f"minimum: {clause.minimum}, maximum: {clause.maximum}"
                            " — BREAKING if changed to 0-100"
                            if "confidence" in clause.field_path
                            else f"range [{clause.minimum}, {clause.maximum}]"
                        )
                    },
                }
            })

        # uuid format → regex test
        if clause.format == "uuid":
            col["tests"].append({
                "dbt_utils.expression_is_true": {
                    "expression": (
                        f"{clause.field_path} ~ "
                    ...
                    "config": {"severity": "error"},
                    "meta": {"contract_clause": "format: uuid"},
                }
            })

        # date-time format → cast test
        if clause.format == "date-time":
            col["tests"].append({
                "dbt_utils.expression_is_true": {
                    "expression": f"{clause.field_path}::timestamp is not null",
                    "config": {"severity": "error"},
                    "meta": {"contract_clause": "format: date-time (ISO 8601)"},
                }
            })
    ...
    dbt_doc = {
        ...
        "models": [
            {
                "name": model_name,
                "description": f"Data contract model for {contract_id}",
                "meta": {
                    "contract_id": contract_id,
                    "bitol_yaml": f"generated_contracts/{contract_id}.yaml",
                },
                "tags": [contract_id.split("-")[0], "data_contract"],
                "columns": columns,
            }
        ],
    }
    ...
    try:
        with open(out_path, "w", encoding="utf-8") as fh:
            ...
            fh.write(f"# dbt schema.yml — {contract_id}\n")
            fh.write(f"# Kept in sync with: generated_contracts/{contract_id}.yaml\n")
            fh.write("# Auto-generated by contracts/generator.py\n")
            fh.write("# Run with: dbt test --select " + model_name + "\n\n")
